...
use ndarray::{
    concatenate, s, Array, Array0, Array2, Array3, ArrayView, ArrayView2, ArrayView3, Axis, Dim,
...
impl Recognition for Whisper {
    ...
    fn inference_logits(
        ...
        let offset = Array0::from_elem((), offset);
        ...
        let offset_id = self.decoder.node_id("offset").unwrap();

        let logits = self.decoder.node_id("logits").unwrap();
        let output_kv_cache_id = self.decoder.node_id("output_kv_cache").unwrap();

        let tokens = as_ndtensor_view(tokens.view()).unwrap();
        let offset = as_ndtensor_view(offset.view()).unwrap();
        let kv_cache = as_ndtensor_view(kv_cache.kv_cache.view()).unwrap();

        // Add the inputs which change on each decoder iteration.
        let mut inputs: Vec<(NodeId, Input)> = vec![
            (tokens_id, tokens.into()),
            (kv_cache_id, kv_cache.into()),
            (offset_id, offset.into()),
        ];
        ...
        let [logits, output_kv_cache] = self
            .decoder
            .run_n(&inputs, [logits, output_kv_cache_id], None)
            .unwrap();

        let logits: NdTensor<f32, 3> = logits.try_into().unwrap();
        let logits = into_array(logits);

        let output_kv_cache: NdTensor<f32, 4> = output_kv_cache.try_into().unwrap();
        let output_kv_cache = into_array(output_kv_cache);

        let new_kv_cache = KVCache {
            kv_cache: output_kv_cache,
        };
